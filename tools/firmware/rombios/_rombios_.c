...
print_bios_banner()
{
  int i = 0;
  for (; i < 1; i++)
    bios_printf(2, "Processor %d: Xen(R) Virtual CPU\n", i+1);
}
  ...
  Bit16u type; Bit32u desc;
{
  ...
  bios_printf(2, "Boot device: %s", drivetypes[type]);
  if (type == 4 && desc != 0) {
  ...
  Bit16u type; Bit8u reason;
{
  ...
  bios_printf(2, " - failure", drivetypes[type]);
  if (type < 4) {
    if (reason==0)
      bios_printf(2, ": disk not bootable");
    else
      bios_printf(2, ": could not read boot disk");
  }
  bios_printf(2, "\n\n");
}
  void
...
void ata_detect( )
{
  ...
  for(device=0; device<(4*2); device++) {
    ...
    if(type == 0x03) {
      ...
      switch (type) {
        case 0x02:
          bios_printf(2, "XS Virtual IDE Controller      Hard Drive");
   if (sizeinmb < (1UL<<16))
            bios_printf(2, " (%uMB)\n", sizeinmb);
   else
            bios_printf(2, " (%uGB)\n", sizeinmb>>10);
          break;
        case 0x03:
          bios_printf(2, "XS Virtual ATAPI-");
          if(read_byte(ebda_seg,&((ebda_data_t *) 0)->ata.devices[device].device)==0x05)
            bios_printf(2, "%d             CD-Rom/DVD-Rom\n",version);
          else
            bios_printf(2, "%d             Device\n",version);
          break;
        case 0x01:
          bios_printf(2, "Unknown device\n");
          break;
        }
      }
    }
